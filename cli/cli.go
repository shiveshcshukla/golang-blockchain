...
func (cli *CommandLine) reindexUTXO(nodeID string) {
	count := lib.ReindexUTXO(nodeID)
	fmt.Printf("Done! There are %d transactions in the UTXO set.\n", count)
}

func (cli *CommandLine) listAddresses(nodeID string) {
	addresses := lib.ListAddresses(nodeID)
...
func (cli *CommandLine) CreateWallet(nodeID string) string {
	address := lib.CreateWallet(nodeID)
...
func (cli *CommandLine) createBlockChain(address, nodeID string) {
	...
	chain, err := blockchain.InitBlockChain(address, nodeID)
	if err != nil {
		log.Panic(err)
		runtime.Goexit()
	}
...
func (cli *CommandLine) getBalance(address, nodeID string) {
	...
	balance := lib.GetBalance(nodeID, address)
...
func (cli *CommandLine) send(from, to string, amount int, nodeID string, mineNow bool) {
	...
	_, err := lib.Send(nodeID, from, to, amount, mineNow)
	if err != nil {
		log.Panic(err)
	}
}
